...
function update_git_info() {
    ...
    FG_COLOR=black
    if [ -n "$_vcs_info" ]; then
        if [ -n "$_git_info_push" ] || [ -n "$_git_info_push" ]; then
            BG_COLOR=yellow
            FG_COLOR=black
        fi
        ...
        echo "%{${bg[$BG_COLOR]}%}%{${fg[$FG_COLOR]}%}$_vcs_info$_git_info_puh$_git_info_pull %{${reset_color}%}%{${reset_color}%}%{${fg[$BG_COLOR]}%}⮀%{${reset_color}%}"
    else
    fi
}
